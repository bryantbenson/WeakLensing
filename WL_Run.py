...
def WL_Run(input_filename, output_prefix, N_halos, z_halo, n_walkers, n_steps, n_burn, n_threads, halo_bounds='default', n_thin=10):
    '''
    '''
    ...
    WLE.WL_Fit(data, bounds_halo, N_halos, z_halo, output_prefix, n_walkers, 
...
WL_Run(input_filename, output_prefix, int(N_halos), float(z_halo), int(n_walkers), int(n_steps), int(n_burn), int(n_threads))